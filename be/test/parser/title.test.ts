...
function input(merchantName: string | null, time: string | null, category: string | null = null) {
...
describe("buildSuggestedTitle", () => {
  ...
  it("buckets morning hours as Breakfast and formats 12h time (food_snack falls through to meal-period)", () => {
    expect(buildSuggestedTitle(input("Solaria", "10:26:06", "food_snack"))).toBe("Breakfast at Solaria on 10:26am");
  });

  it("buckets midday hours as Lunch", () => {
    expect(buildSuggestedTitle(input("Solaria", "12:30", "food_snack"))).toBe("Lunch at Solaria on 12:30pm");
  });

  it("buckets evening hours as Dinner, formats pm correctly", () => {
    expect(buildSuggestedTitle(input("Solaria", "19:00", "food_snack"))).toBe("Dinner at Solaria on 07:00pm");
  });

  it("buckets late hours as Late night snack", () => {
    expect(buildSuggestedTitle(input("Solaria", "23:15", "food_snack"))).toBe("Late night snack at Solaria on 11:15pm");
  });
  ...
  it("falls back to meal-period phrasing when category is omitted/unclassified", () => {
    expect(buildSuggestedTitle(input("Solaria", "12:30", null))).toBe("Lunch at Solaria on 12:30pm");
  });

  it("uses a fixed label for grocery regardless of time of day", () => {
    expect(buildSuggestedTitle(input("Indomaret", "16:04", "grocery"))).toBe("Groceries at Indomaret on 04:04pm");
  });

  it("uses a fixed label for bill/tax payment", () => {
    expect(buildSuggestedTitle(input("PLN", "09:00", "bills"))).toBe("Bill payment at PLN on 09:00am");
  });
  ...
  it("falls back to the single item's name when there's no merchant", () => {
    expect(
      buildSuggestedTitle({ merchant: { name: null }, transaction: { time: null }, metadata: { category: null }, items: [{ name: "Taxi" }] }),
    ).toBe("Expense: Taxi");
  });

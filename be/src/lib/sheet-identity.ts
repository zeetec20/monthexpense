...
async function hmacKey(signingKey: string): Promise<CryptoKey> {
  return crypto.subtle.importKey("raw", new TextEncoder().encode(signingKey), { name: "HMAC", hash: "SHA-256" }, false, [
    "sign",
  ]);
}

export async function computeHash(signingKey: string, payload: string): Promise<string> {
...
export async function verifySheetSecret(
  secret: string,
  ...
  if (timingSafeEqual(hashPart, await computeHash(keys.standard, uuid))) return { uuid, tier: "standard" };
  if (timingSafeEqual(hashPart, await computeHash(keys.premium, uuid))) return { uuid, tier: "premium" };
  return null;
}

...
import { DAILY_LIMIT, usageKey, type Group, type IdentityBackend } from "../middleware/identity-quota";
...
function realPeek(env: HonoEnv["Bindings"]): IdentityBackend["peekUsage"] {
...
export const getQuotaStatus = async (c: Context<HonoEnv>) => {
  ...
  const entries = await Promise.all(
    GROUPS.map(async (group) => [group, { remaining: Math.max(limit - (await peek(usageKey(identity.uuid, group))), 0), limit }] as const),
  );

...
import { differenceInCalendarDays, getDaysInMonth, parseISO, startOfMonth, startOfYear, subMonths } from "date-fns";
...
function bucketByFiveDays(expenses: Expense[], today: Date) {
...
function bucketByMonth(expenses: Expense[], lang: Lang) {
  ...
  return [...sums.entries()]
    ...
    .map(([key, spending]) => ({
      name: new Date(`${key}-01`).toLocaleDateString(lang === "id" ? "id-ID" : "en-US", { month: "short" }),
      spending,
    }));
}

// isCountedExpense: a settled Hutang doesn't count as real spend anymore.
function periodExpenses(expenses: Expense[], period: Period, today: Date) {
  ...
  if (period === "thisYear") return counted.filter((e) => e.date.startsWith(String(today.getFullYear())));
...
function previousPeriodExpenses(expenses: Expense[], period: Period, today: Date) {
  const counted = expenses.filter(isCountedExpense);
  if (period === "thisMonth") {
...
function periodDays(period: Period, today: Date) {
 ...
 */
export function AnalyticsPage({ expenses, lang }: { expenses: Expense[]; lang: Lang }) {
  const [period, setPeriod] = useState<Period>("thisMonth");
  const today = useMemo(() => new Date(), []);
  const currency = expenses[0]?.currency ?? "IDR";

  const inPeriod = useMemo(() => periodExpenses(expenses, period, today), [expenses, period, today]);
  ...
  const categoryData = useMemo(
    () =>
      EXPENSE_CATEGORIES.map((c) => ({
        ...
        value: inPeriod.filter((e) => (e.category ?? "other") === c.value).reduce((sum, e) => sum + e.amount, 0),
  ...
  const peak = trendData.length > 0 ? trendData.reduce((a, b) => (b.spending > a.spending ? b : a)) : null;
  ...
  return (
    <div className="space-y-3.5 max-w-sm mx-auto">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="p-3.5 rounded-2xl bg-card border border-line transition-colors duration-200">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] font-semibold text-ink-faint">{t(lang, "totalSpending")}</span>
          ...
          <p className="text-base font-extrabold text-ink font-mono">{formatCurrency(totalSpent, currency)}</p>
        </div>
        <div className="p-3.5 rounded-2xl bg-card border border-line transition-colors duration-200">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] font-semibold text-ink-faint">{t(lang, "dailyAverage")}</span>
            <Calendar className="w-3.5 h-3.5 text-emerald-500 dark:text-emerald-400" />
          </div>
          <p className="text-base font-extrabold text-ink font-mono">{formatCurrency(dailyAverage, currency)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="p-2.5 rounded-2xl bg-card border border-line flex items-center gap-2.5 transition-colors duration-200">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] font-bold text-amber-600 dark:text-amber-400 font-mono">{peak?.name ?? "–"}</p>
          </div>
        </div>
      </div>

      <div className="p-4 rounded-3xl bg-card border border-line space-y-2 shadow-lg transition-colors duration-200">
        <h3 className="text-xs font-bold text-ink uppercase tracking-wider">{t(lang, "spendingTrend")}</h3>
        {trendData.length === 0 ? (
          ...
          <div className="h-44 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trendData} margin={{ top: 10, right: 5, left: -25, bottom: 0 }}>
                ...
                <XAxis dataKey="name" stroke="var(--color-ink-3)" fontSize={9} tickLine={false} axisLine={false} />
                <YAxis
                ...
                />
                <Area type="monotone" dataKey="spending" stroke="#7FA873" strokeWidth={2.5} fill="url(#emeraldG)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        )}
      ...
      <div className="p-4 rounded-3xl bg-card border border-line space-y-2 shadow-lg transition-colors duration-200">
        <h3 className="text-xs font-bold text-ink uppercase tracking-wider">{t(lang, "categoryBreakdown")}</h3>
        {totalSpent === 0 ? (
          <p className="py-8 text-center text-xs text-ink-faint">{t(lang, "noDataYet")}</p>
        ) : (
              ...
              <PieChart>
                <Pie data={pieData} cx="50%" cy="50%" innerRadius={45} outerRadius={70} paddingAngle={3} dataKey="value">
                  {pieData.map((entry, index) => (
                    <Cell key={index} fill={entry.color} stroke="var(--color-paper)" strokeWidth={2} />
                  ))}
                </Pie>
                <Tooltip
                  formatter={(value) => formatCurrency(Number(value) || 0, currency)}
        ...
        <div className="grid grid-cols-2 gap-1.5 pt-1 text-xs">
          {categoryData.map((c) => (
            <div key={c.name} className="flex items-center justify-between p-1 rounded-lg bg-elevated/60">
              <div className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: c.color }} />
                <span className="text-ink-soft text-[10px] truncate max-w-[80px]">{c.name}</span>
              </div>
              <span className="font-mono text-ink-faint text-[10px]">{formatCurrency(c.value, currency)}</span>
            </div>
          ))}
        </div>
      </div>

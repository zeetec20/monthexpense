...
export const ExpenseReviewModal = ({
  ...
  // genuinely couldn't tell. Derived during render (React's "adjusting
  // state when a prop changes" pattern) instead of an effect; `seed` tracks
  // the (initial, defaultWalletId) pair already applied to `draft`.
  const [seed, setSeed] = useState({ initial, defaultWalletId });
  if (initial !== seed.initial || defaultWalletId !== seed.defaultWalletId) {
    setSeed({ initial, defaultWalletId });
    setDraft({
      ...initial,
      walletId: initial.walletId ?? defaultWalletId,
      category: initial.category ?? "food_snack",
    });
    setEditing(false);
  }
  ...
  const updateDetail = (patch: Partial<ExpenseReceiptDetail>) => {
    setDraft((d) => ({
      ...d,
      receiptDetail: { ...(d.receiptDetail ?? BLANK_RECEIPT_DETAIL), ...patch },
    }));
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {validationWarning && (
        <p className="text-sm text-[var(--color-warning,#a15c00)]">{validationWarning[lang]}</p>
      )}
      ...
      {!editing && !detail && (
        <Row
          ...
          value={formatCurrency(draft.amount, draft.currency)}
          strong
        />
      )}
        ...
        <>
          ...
          <div className="space-y-1.5 border-t border-[var(--color-rule)] pt-3">
            {totals?.subtotal !== null && (
              <Row
                ...
                value={formatCurrency(totals?.subtotal ?? null, draft.currency)}
              />
            )}
            <SummaryField
            ...
            <Row
              ...
              value={formatCurrency(displayAmount, draft.currency)}
              strong
            />
          </div>
          ...
          {(editing || detail.paymentMethod) && (
            <div className="space-y-1.5 border-t border-[var(--color-rule)] pt-3">
              {editing ? (
                <>
                  <Label htmlFor="expense-review-payment-method">

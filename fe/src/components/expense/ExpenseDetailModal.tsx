...
const SOURCE_LABEL_KEY: Record<
  Expense["source"],
  "sourceScanned" | "sourceManual" | "sourceVoice"
> = {
  scan: "sourceScanned",
  manual: "sourceManual",
  voice: "sourceVoice",
...
export const ExpenseDetailModal = ({
  ...
  // further down) still keys off the live `expenseProp`. Derived during
  // render (React's "adjusting state when a prop changes" pattern)
  // instead of a ref read during render.
  const [expense, setExpense] = useState<Expense | null>(expenseProp);
  if (expenseProp && expenseProp !== expense) setExpense(expenseProp);
  ...
  // Re-seed the input whenever a different expense is opened (id or its
  // title changes) — skipped while closing (expenseProp null) so the
  // header doesn't flash blank mid-animation. `titleSeed` tracks the last
  // (id, title) pair already applied to `title`, so this doesn't re-fire
  // just because the user edited `title` away from expenseProp.title.
  const [titleSeed, setTitleSeed] = useState<{ id: string; title: string } | null>(
    expenseProp ? { id: expenseProp.id, title: expenseProp.title } : null,
  );
  if (expenseProp && (expenseProp.id !== titleSeed?.id || expenseProp.title !== titleSeed.title)) {
    setTitleSeed({ id: expenseProp.id, title: expenseProp.title });
    setTitle(expenseProp.title);
  }
  ...
  useEffect(() => {
    ...
    if (!imageId) {
      // eslint-disable-next-line react/set-state-in-effect -- shares this id-keyed effect (and its cancellation cleanup) with the real async getImage() fetch below; not worth forking the loading/none/loaded tri-state into render-time derivation too
      setImageUrl(null);
      return;
    }
  ...
  return (
    <>
      <BottomSheet
        open={expenseProp !== null}
        onClose={onClose}
        title={t(lang, "expenseDetailTitle")}
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto -mx-1 px-1 space-y-4">
              ...
              <div className="space-y-1.5 border-t border-[var(--color-rule)] pt-3">
                ...
                {detail.discount !== null && detail.discount !== 0 && (
                  <Row
                    ...
                    value={`-${formatCurrency(detail.discount, currency)}`}
                  />
                )}
                {detail.tax !== null && (
                  <Row label={t(lang, "taxLabel")} value={formatCurrency(detail.tax, currency)} />
                )}
                {detail.serviceCharge !== null && detail.serviceCharge !== 0 && (
                  <Row
                    label={t(lang, "serviceChargeLabel")}
                    value={formatCurrency(detail.serviceCharge, currency)}
                  />
                )}
                <Row
                  label={t(lang, "totalLabel")}
              ...
              {(detail.paymentMethod || detail.cashReceived !== null || detail.change !== null) && (
                <div className="space-y-1.5 border-t border-[var(--color-rule)] pt-3 text-sm">
                  ...
                  {detail.cashReceived !== null && (
                    <Row
                      label={t(lang, "cashLabel")}
                      value={formatCurrency(detail.cashReceived, currency)}
                    />
                  )}
                  {detail.change !== null && (
                    <Row
                      label={t(lang, "changeLabel")}
                      value={formatCurrency(detail.change, currency)}
                    />
                  )}
                </div>
              )}
          ...
          <p className="text-xs text-[var(--color-ink-3)]">
            {t(lang, "addedOn", { date: formatDate(expense.createdAt) })}
          </p>

...
const { readAll, writeAll, getRemoveBlockReason, addWalletPure, renameWalletPure, updateWalletAnimalPure, removeWalletPure } =
  await import("./wallet.store");
const { WALLET_MAX, DEFAULT_WALLET_NAME, WALLET_ANIMALS, randomAnimal } = await import("./wallet.schema");
...
test("writeAll then readAll round-trips a wallet list", () => {
  const wallets = [{ id: "1", name: "Cash", animal: "cat" as const }, { id: "2", name: "Bank", animal: "dog" as const }];
  writeAll(wallets);
...
test("addWalletPure refuses past WALLET_MAX", () => {
  const full = Array.from({ length: WALLET_MAX }, (_, i) => ({ id: String(i), name: `W${i}`, animal: "cat" as const }));
...
test("renameWalletPure trims and updates the matching wallet only", () => {
  const wallets = [{ id: "1", name: "Main", animal: "cat" as const }, { id: "2", name: "Bank", animal: "dog" as const }];
  const next = renameWalletPure(wallets, "2", "  Savings  ");
  expect(next).toEqual([{ id: "1", name: "Main", animal: "cat" }, { id: "2", name: "Savings", animal: "dog" }]);
});
...
test("updateWalletAnimalPure updates the matching wallet's icon only", () => {
  ...
  expect(next).toEqual([{ id: "1", name: "Main", animal: "cat" }, { id: "2", name: "Bank", animal: "turtle" }]);
});
...
test("getRemoveBlockReason explains why, or is null when removal is safe", () => {
  ...
  expect(getRemoveBlockReason("2", wallets, [{ walletId: "2" } as never])).toMatch(/reassign|rename/i);

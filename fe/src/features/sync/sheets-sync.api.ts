...
import { TXN_HEADERS, WALLET_HEADERS, TRANSACTIONS_SHEET_ID, CONFIG_SHEET_ID } from "./google-provision.client";
...
export function readCredentials(): SheetCredentials | null {
  try {
    ...
    return { secret: parsed.secret, spreadsheetId: parsed.spreadsheetId, email: typeof parsed.email === "string" ? parsed.email : undefined };
  } catch {
    return null;
  }
}

export function writeCredentials(credentials: SheetCredentials): void {
...
export function sheetIdentityHeaders(): Record<string, string> {
...
export async function validateSheetSecret(secret: string, spreadsheetId: string): Promise<void> {
  ...
  if (!response.ok) {
    ...
    throw new Error(body?.error?.message || "This sync secret wasn't issued by this app. Reconnect with a valid one.");
  }
}
...
function dateToSerial(dateKey: string): number {
...
function sheetGoneError(status: number): Error & { code: string } {
  const err = new Error(`The connected spreadsheet appears to have been deleted (${status}).`) as Error & { code: string };
...
async function sheetsFetch(url: string, init: RequestInit = {}, isRetry = false): Promise<any> {
...
function walletRow(wallet: Wallet): (string | number)[] {
  return [wallet.id, wallet.name];
}

function walletFromRow(row: unknown[]): Wallet {
  return { id: String(row[0] ?? ""), name: String(row[1] ?? ""), animal: "cat" };
}

function txnRow(expense: Expense, walletNameById: Map<string, string>): (string | number)[] {
...
function parseDetailCell(cell: unknown): Pick<Expense, "receiptDetail" | "scheduleType" | "paid" | "settled"> {
...
function expenseFromRow(row: unknown[]): Expense {
...
async function findRow(spreadsheetId: string, sheetName: string, id: string): Promise<number> {
...
async function buildWalletNameMap(spreadsheetId: string): Promise<Map<string, string>> {
  const json = await sheetsFetch(
...
async function upsertExpense(spreadsheetId: string, expense: Expense): Promise<void> {
...
async function deleteExpense(spreadsheetId: string, id: string): Promise<void> {
  ...
  await sheetsFetch(`${SHEETS_API}/${spreadsheetId}:batchUpdate`, {
    ...
    body: JSON.stringify({
      requests: [
        { deleteDimension: { range: { sheetId: TRANSACTIONS_SHEET_ID, dimension: "ROWS", startIndex: row - 1, endIndex: row } } },
      ],
    }),
  });
}

async function upsertWallet(spreadsheetId: string, wallet: Wallet): Promise<void> {
  ...
  if (row === -1) {
    ...
    await sheetsFetch(`${SHEETS_API}/${spreadsheetId}/values/Config!A:B:append?valueInputOption=RAW&insertDataOption=OVERWRITE`, {
      method: "POST",
      body: JSON.stringify({ values }),
    });
  } else {
    await sheetsFetch(`${SHEETS_API}/${spreadsheetId}/values/Config!A${row}:B${row}?valueInputOption=RAW`, {
      method: "PUT",
      body: JSON.stringify({ values }),
    });
  }
...
async function deleteWallet(spreadsheetId: string, id: string): Promise<void> {
  const row = await findRow(spreadsheetId, "Config", id);
  if (row === -1) return;
  await sheetsFetch(`${SHEETS_API}/${spreadsheetId}:batchUpdate`, {
    method: "POST",
    body: JSON.stringify({
      requests: [{ deleteDimension: { range: { sheetId: CONFIG_SHEET_ID, dimension: "ROWS", startIndex: row - 1, endIndex: row } } }],
    }),
  });
}
...
export async function applySyncOp(spreadsheetId: string, op: SyncOp): Promise<void> {
...
async function batchGetAll(spreadsheetId: string): Promise<PulledData> {
  const json = await sheetsFetch(
...
export async function pullAll(spreadsheetId: string): Promise<PulledData> {
...
export async function pullRecent(spreadsheetId: string, cutoffKey: string): Promise<PulledData> {
...
export async function pushAll(spreadsheetId: string, expenses: Expense[], wallets: Wallet[]): Promise<void> {
  ...
  if (expenses.length > 0) data.push({ range: "Transactions!A2", values: expenses.map((e) => txnRow(e, walletNameById)) });
  if (data.length > 0) {

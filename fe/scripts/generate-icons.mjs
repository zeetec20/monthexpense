...
const roundedRectMask = (size, radiusRatio = 0.25) => {
...
await writeFile(
  ...
  await sharp(master).resize(192, 192).png().toBuffer(),
);
await writeFile(
  path.join(publicDir, "icon-512.png"),
  await sharp(master).resize(512, 512).png().toBuffer(),
);
await writeFile(
  path.join(publicDir, "favicon.png"),
  await sharp(master).resize(64, 64).png().toBuffer(),
);
...
const fullBleed = await solidSquare(SIZE, BRAND)
...
await writeFile(
  ...
  await sharp(fullBleed).resize(180, 180).flatten({ background: BRAND }).png().toBuffer(),
);
...
await writeFile(
  ...
  await solidSquare(512, BRAND)
    .composite([
      {
        input: await sharp(fullBleed).resize(maskableInset, maskableInset).toBuffer(),
        gravity: "center",
      },
    ])
    .png()
...
console.log(
  "Generated icon-192.png, icon-512.png, maskable-icon-512.png, apple-touch-icon.png, favicon.png in public/",
);
